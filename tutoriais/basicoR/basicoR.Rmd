...
* **Help** quem não precisa de uma help? É nessa aba que podemos fazer buscas
sobre ajudas nos pacotes. Na realidade, recomendamos que sejam observadas as
documentações de cada pacote que usem.

* **Viewer** Nesse local são aprensentadas tabelas e gráficos em formato HTML.
...
# vamos ver quantas fatias temos para as 7 criaças
...
do tiplo "numeric" pelo R. O usuário precisará indicar que o dados é do tipo
inteiro.
...
Agora que sabemos criar um novo objeto e entendeos o que faz a função c(),
...
pesquisas também são desenvolvida com aplicação de questionários ou entrevistas
...
ser usados pelo usuários como uma matriz de valores. Então, apresentamos o nosso
...
e passivo circulante que são variaveis numéricas (numeric). É com esse tipo de 
...
função "rep()" com o parâmetro de sequencia "1:6" para indicar os número de 1 a 6.
...
# Criamos um data frame com a primeira coluna sendo codigos, a segund coluna
...
Observe que as 6 primeiras obsevações tem valor "TRUE" e as demais valor "FALSE".
...
Vamos ver a nova esturutra com a função "str()" e seu tipo com a função "class()".
...
Criamos, utilizando a plataforma Economatica, uma banco de dados com o ativo
...
regra de como um dado tenha que ser analisado. O que precisamos é enteder o
...
Vimos que as três maiores liquidez são de GPAR3, e AFLT3 (2 vezes). Para ver
as empresas com maior liquidez corrente por ano vamos para a próxima pergunta
desta questão.
